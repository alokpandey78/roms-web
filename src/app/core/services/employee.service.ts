...
export class EmployeeService {
  ...
  getAll(options: ViewOptions) {
    return this.http.get<any>(
      `${environment.apiUrl}/v1/employee/load?page=${options.page}&size=${options.pageSize}&${options.query}`,
    );
  }
  ...
  getAllResignationRequest(options: ViewOptions) {
    return this.http.get<any>(
      `${environment.apiUrl}/v1/resignation/appliedToMe?status=${options.search}&page=${options.page}&size=${options.pageSize}`,
    );
  }

  resignationHistory(options: ViewOptions) {
    return this.http.get<any>(
      `${environment.apiUrl}/v1/resignation/appliedToMeHistory?page=${options.page}&size=${options.pageSize}`,
    );
  }
  ...
  employeeOnboardList(options: ViewOptions) {
    return this.http.get<any>(
      `${environment.apiUrl}/v1/employee/onboard/loadAll?page=${options.page}&size=${options.pageSize}`,
    );
  }

  getAllEmployeeResignation(options: ViewOptions, queryData: any) {
    return this.http.post<any>(
      `${environment.apiUrl}/v1/resignation/loadAll?page=${options.page}&size=${options.pageSize}`,
      queryData,
    );
  }
  ...
  searchEmployeeForJobDemand(value: string) {
    return this.http.get<any>(
      `${environment.apiUrl}/v1/employee/load?page-0&size=800&empName=${value}`,
    );
  }
  ...
  getEmployeeList(options: ViewOptions, payload: any) {
    return this.http.get<any>(
      `${environment.apiUrl}/v1/employee/load?page=${options.page}&size=${options.pageSize}&empName=${payload.searchText}&payfrequency=${payload.frequency}&empStatus=${payload.empStatus}`,
    );
  }
